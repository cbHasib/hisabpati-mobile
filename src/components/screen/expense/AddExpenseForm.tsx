...
import { useCreateExpenseMutation } from '@/src/redux/api.config/expenseApi';
...
export default function AddExpenseForm({ onSuccess }: Props) {
  ...
  const onSubmit = async (data: ExpenseIncomeFormData) => {
    try {
      ...
      Alert.alert('Error', err?.data?.message ?? 'Failed to add expense.');
    }
  };
  ...
  return (
    <View>
      ...
      />
      <Button title="Add Expense" onPress={handleSubmit(onSubmit)} loading={isLoading} variant="danger" />
    </View>
  );
}
